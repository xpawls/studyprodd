...
select * from emp, (select deptno from dept where loc='인천') p 
...
select ename, hiredate, deptno from emp where deptno in(select deptno from dept where dname='영업부');



-- 2. 과장의 최소 급여보다 많은 급여를 받는 직원들의 
--    이름, 급여 그리고 직급을 출력하되, 과장은 출력하지 않는다.
select * from emp where sal>(select min(sal) from emp where job='과장') and job!='과장';
...
select emp.*, sal+100 from emp where deptno in(select deptno from dept where loc='인천');
